...
function formatSaldo(saldo: string | null): string {
...
export function FinanzasPage() {
  ...
  const fetchAll = useCallback(async () => {
    try {
      const [ambitosRes, cuentasRes] = await Promise.all([
      ...
      setAmbitos(ambitosData.ambitos ?? [])
  ...
  return (
    <div style={{ flex: 1, overflowY: 'auto', padding: '24px 32px', background: '#0D0A06', display: 'flex', flexDirection: 'column', gap: 32 }}>
      <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between' }}>
        <div>
          ...
          <p style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 11, color: '#5A4A30', margin: '4px 0 0' }}>
            Cuentas financieras por ámbito
          </p>
        </div>
        <button
          onClick={() => openCreate()}
